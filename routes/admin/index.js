...
router.post("/logo-change",ensureAuthenticated,adminAuth,async(req,res)=>{
  let user=req.user
  if(!user){
    req.flash("error_msg","please re-login")
    res.redirect("/users/login")
...
router.get("/search-product",ensureAuthenticated,adminAuth, async (req, res) => {
  try {
    req.session.currentUrl=req.originalUrl
...
router.post("/order-delivered/:id", ensureAuthenticated,adminAuth,async (req, res) => {
...
${orderMsg}
  ...
  let sms="Hello,we just sent you the inventory report of your purchase through your email,thank you for buying from us"
...
router.post("/order-not-delivered/:id",ensureAuthenticated, adminAuth,async(req, res) => {
...
router.post("/donko-to-previous/:id",ensureAuthenticated,adminAuth,async (req, res) => {
  const product = await Product.findById(req.params.id);
  product.isFiftyOff = false;
  product.discount=0;
  product.price =product.oldPrice;
  await product.save();
  req.flash("Successfully removed " + product.name + " from 50% off products");
  res.redirect(req.headers.referer);
});
...
router.get("/discount/edit/:id",ensureAuthenticated, adminAuth,async(req, res) => {
  req.session.currentUrl=req.originalUrl
...
router.post("/discount/edit/:id",ensureAuthenticated,adminAuth,async (req, res) => {
  let product=await Product.findById(req.params.id)
  
  if(parseInt(req.body.discount)==0){
    //no discount
        let noDiscount=  await Product.findByIdAndUpdate({_id: req.params.id},{
      $set:{
          discount:0,
    ...
    let fiftyPriceUpdate=  await Product.findByIdAndUpdate({_id: req.params.id},{
      $set:{
          discount:50,
          price:product.oldPrice - (parseInt(req.body.discount) / 100) * product.oldPrice,
...
//DELETING coupon
router.delete("/coupon/:id",ensureAuthenticated, adminAuth,async (req, res) => {
...
//Add product
router.get("/add-category",ensureAuthenticated, adminAuth,async(req, res) => {
  req.session.currentUrl=req.originalUrl
...
// edit category
router.get("/category/edit/:id",ensureAuthenticated,adminAuth, async(req, res) => {
  req.session.currentUrl=req.originalUrl
...
router.get("/edit/category",ensureAuthenticated, adminAuth,async(req, res) => {
  let lOgo=await Logo.findOne({});
      let siteLogo=lOgo?lOgo.url:" "
    req.session.currentUrl=req.originalUrl
...
//update category
router.put("/category/edit/:id",ensureAuthenticated,adminAuth, async(req, res) => {
  let lOgo=await Logo.findOne({});
      let siteLogo=lOgo?lOgo.url:" "
  ...
  if (validation.fails()) {
    ...
    res.render("admin/addcategory", { errors, hero, addresS,siteLogo,
      user });
  }
  validation.passes(async () => {
    try {
...
// delete category
router.delete("/category/delete/:id",ensureAuthenticated,adminAuth,async (req, res) => {
  const { id } = req.params;
...
router.post("/add-category",ensureAuthenticated,adminAuth,async (req, res) => {
  let lOgo=await Logo.findOne({});
      let siteLogo=lOgo?lOgo.url:" "
  ...
  const orderCount = await Order.countDocuments();
    ...
    res.render("admin/addcategory", {siteLogo,
      user, errors, hero, ad50, addresS,pcount,subCount,cCount,fiftyOffProductsCount,orderCount });
  } else {
    validation.passes(async () => {
      try {
        const uploader = async (path) =>
...
router.get("/products/all",ensureAuthenticated,adminAuth,async (req, res) => {
  req.session.currentUrl=req.originalUrl
...
//edit a product
router.get("/product/:id/edit/",ensureAuthenticated,adminAuth, async(req, res) => {
  let lOgo=await Logo.findOne({});
      let siteLogo=lOgo?lOgo.url:" "
    req.session.currentUrl=req.originalUrl
...
router.post("/product/:id/edit/",ensureAuthenticated,adminAuth,async (req, res) => {
  let lOgo=await Logo.findOne({});
      let siteLogo=lOgo?lOgo.url:" "
      let product=await Product.findById(req.params.id)
...
router.delete("/product/:id/edit/", ensureAuthenticated,adminAuth,async(req, res) => {
...
router.get("/donko-products",ensureAuthenticated,adminAuth, async(req, res) => {
  req.session.currentUrl=req.originalUrl
...
//single order
router.get("/order-info/:id",ensureAuthenticated,adminAuth,async (req, res) => {
  let lOgo=await Logo.findOne({});
      let siteLogo=lOgo?lOgo.url:" "
    req.session.currentUrl=req.originalUrl
...
//printing invoice
router.get("/order-info-print/:id",ensureAuthenticated,adminAuth, async(req, res) => {
  let lOgo=await Logo.findOne({});
      let siteLogo=lOgo?lOgo.url:" "
    req.session.currentUrl=req.originalUrl
  ...
  res.render("admin/order-print", {
    ...
    ad50,
    addresS,
    fiftyOffProductsCount,subCount,siteLogo,
    user
  });
});
...
router.post("/hero-edit/:id",ensureAuthenticated,adminAuth, async(req, res) => {
  try {
    let lOgo=await Logo.findOne({});
      let siteLogo=lOgo?lOgo.url:" "
...
router.delete("/delete-hero/:id",ensureAuthenticated,adminAuth, async(req, res) => {
  try {
...
router.post("/edit-ad/:id",ensureAuthenticated,adminAuth, async(req, res) => {
  let lOgo=await Logo.findOne({});
  let siteLogo=lOgo?lOgo.url:" "
  let user=req.user
  ...
  if (validation.fails()) {
    ...
    const pcount = await Product.countDocuments();
    const orderCount = await Order.countDocuments();
    const fiftyOffProductsCount = await Product.find({isFiftyOff: true});
  const subCount=await Mail.count({})
    ...
    ad.adImg = newPath;
    ad = await ad.save();
    req.flash("success_msg", "Succesfully updated Ad");
    res.redirect("/admin/");
  }
});

router.delete("/delete-ad/:id",ensureAuthenticated,adminAuth,async (req, res) => {
...
router.post("/add-address",ensureAuthenticated,adminAuth, async(req, res) => {
  let lOgo=await Logo.findOne({});
      let siteLogo=lOgo?lOgo.url:" "
...
router.get("/edit-address",ensureAuthenticated, adminAuth,async(req, res) => {
  let lOgo=await Logo.findOne({});
      let siteLogo=lOgo?lOgo.url:" "
    req.session.currentUrl=req.originalUrl
...
router.post("/edit-address",ensureAuthenticated, adminAuth,async(req, res) => {
  let lOgo=await Logo.findOne({});
      let siteLogo=lOgo?lOgo.url:" "
...
router.delete("/delete-address/:id",ensureAuthenticated,adminAuth, async(req, res) => {
...
.skip(itemPerPage * currentPage - itemPerPage)
  ...
  res.render("admin/subscribers",{subCount, hero,
    cCount,
    pcount,
    orderCount,
    ad50,
    ...
    fiftyOffProductsCount,siteLogo,
    user
 })
})
